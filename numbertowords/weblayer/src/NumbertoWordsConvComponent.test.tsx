...
describe('NumberToWordsConverter', () => {
    ...
    test('converts one number to words correctly', async () => {
        render(<NumbertoWordsConvComponent />);
        
        global.fetch = jest.fn(() =>
            Promise.resolve({
              json: () => Promise.resolve({ result: 'ONE DOLLAR' }),
            })
        ) as jest.Mock;
        ...
        await waitFor(() => {
            const alertElement = screen.getByRole('alert');
            expect(alertElement).toHaveTextContent('ONE DOLLAR');
        });
    });

    test('converts negative number to words correctly by negating the sign', async () => {
        render(<NumbertoWordsConvComponent />);
