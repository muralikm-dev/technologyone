...
class NumtowordsconvertorApplicationTests {
    ...
    @Test
    void testOneDollarToWords() {
        long number = 1L;
        String result = numberToWordsConvController.numberToWordsConvertion(number);
        assertThat(result).isEqualTo("{\"result\": \"ONE DOLLAR\"}");
    }

    @Test
    void testNegativeNumberToWords() {
        double number = -123.60;
        String result = numberToWordsConvController.numberToWordsConvertion(number);
        assertThat(result).isEqualTo("{\"result\": \"ONE HUNDRED AND TWENTY-THREE DOLLARS AND SIXTY CENTS\"}");
    }
